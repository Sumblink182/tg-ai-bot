...
<p align="center">
  <b>没有 API Key？只有一台 1 核小鸡？照样拥有一个真能干活的 Telegram AI Agent。</b><br>
...
## 🥊 与同类开源项目对比（截至 2026-09）

| | karfly/chatgpt_telegram_bot (5.5k⭐) | OpenClaw (180k+⭐) | 多模型对话类 Bot | **本项目** |
| :--- | :--- | :--- | :--- | :--- |
| **需要自备 API Key** | ✅ 必须（OpenAI 计费） | ✅ 需要 | ✅ 必须 | ❌ **不需要，Google 账号 OAuth 即用** |
| **模型使用成本** | 按 token 计费 | 按 token 计费 / 本地模型 | 按 token 计费 | 💡 **免费额度，0 成本** |
| **真实执行能力** | ❌ 仅对话 | ✅ 完整 Agent（插件生态） | ❌ 仅对话 | ✅ 宿主机级 Agent（白名单+二次确认） |
| **最低资源要求** | Docker 全套 + 数据库 | 重型（Node 全家桶） | 轻量 | ⚡ **~20MB 内存，1C1G 小鸡可跑** |
| **接入平台** | Telegram | 20+ 平台 | Telegram | Telegram |
| **适合谁** | 已有 API Key、求成熟稳定 | 预算充足、要多平台生态 | 多模型对话党 | **没有 Key、只有小鸡、要真能干活** |

> [!WARNING]
> **坦诚说明**：本项目上游为 Google Antigravity CLI 的免费模型额度，**非官方商用 API**，模型可用性与配额政策由 Google 掌控，不保证长期不变。生产关键业务请自备官方 API Key 并切换到内置的备用 OpenAI 兼容引擎。介意请勿依赖。
